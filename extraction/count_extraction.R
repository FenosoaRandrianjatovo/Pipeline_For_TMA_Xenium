...
# 2. Sample 3000 cell barcodes (make reproducible with set.seed)
set.seed(123)
selected_cells <- sample(colnames(counts_mat), 50000)

# 3. Subset the matrix to those 3000 cells (genes stay the same)
counts_sub <- counts_mat[, selected_cells]

# 4. Transpose & coerce to dense data.frame
counts_df <- as.data.frame(t(as.matrix(counts_sub)))

# 5. Add the corresponding cluster labels
